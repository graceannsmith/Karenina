const bookText = `Happy families are all alike; every unhappy family is unhappy in its own way.
...
function getRandomQuote() {
  const randomIndex = Math.floor(Math.random() * bookSentences.length);
  let theQuote = bookSentences[randomIndex] + '.';
  return theQuote;
}
...
window.onload = function () {
  ...
  form.onsubmit = function (event) {
    ...
    let resultText = getRandomQuote();
    form.reset();
    document.querySelector("p").innerText = resultText;
